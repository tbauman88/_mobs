...
const WeekCard: React.FC<{ session: Session }> = ({ session }) => {
  const {
    attendee_limit,
    attendees,
    end_time,
  ...
  return (
    <>
      <IonCard
        className="week-card ion-margin-vertical"
        // onClick={() => setSession(session)}
        // routerLink={`/tabs/sessions/${id}`}
      >
        <IonCardHeader style={{ paddingRight: isTodayOrFutrue && '4px' }}>
          <IonItem detail={false} lines="none" className="ion-align-items-start ion-no-padding">
          ...
          >
            <IonGrid className="ion-no-padding">
              ...
              <IonRow className="ion-align-items-center ion-justify-content-between">
                ...
                {isTodayOrFutrue && <IonButton>Join</IonButton>}
              </IonRow>
            </IonGrid>
          </IonItem>
        </IonCardContent>
